...
loadsPo(){
  if (this.searchKey === false && (this.text ==''|| this.text ===null) ){
    this.poservice.GetPo(this.pagination.currentPage, this.pagination.itemsPerPage)
    .subscribe((res: PaginatedResult<Po[]>) => {
      this.pos = res.result;
    ...
    this.poservice.SeachPo(this.pagination.currentPage, this.pagination.itemsPerPage,this.text)
    .subscribe((res: PaginatedResult<Po[]>)  => {
      this.pos = res.result;
      this.pagination = res.pagination;
...
deletePo(po: Po) {
  this.alertify.confirm('Delete Po', 'Are you sure you want to delete this ID "' + po.id + '" ?', () => {
